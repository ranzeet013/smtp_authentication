...
from schemas.user_schemas import ChangePassword, UserCreate, UserLogin, UserOut, Token, OTPVerify, APIResponse
...
async def ping():
    ...
    """
    return {"status": "success", "data": {"message": "pong from file_knowledgebase"}}

@router.post("/register", response_model=APIResponse[UserOut], tags=["Authentication"])
async def register(user: UserCreate, db: AsyncSession = Depends(get_db)):
    """
    Register a new user and send OTP for email verification.
    Request Body: UserCreate (name, email, password)
    Response: APIResponse with UserOut (uuid, name, email, is_verified)
...
@router.post("/verify-otp", response_model=APIResponse[UserOut], tags=["Authentication"])
async def verify_otp_endpoint(otp_data: OTPVerify, db: AsyncSession = Depends(get_db)):
    ...
    """
    user = await verify_otp(db, otp_data)
...
async def login(user: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    Response: APIResponse with Token (access_token, token_type)
...
@router.get("/users/me", response_model=APIResponse[UserOut], tags=["Authentication"])
...
@router.delete("/users/me", response_model=APIResponse[None], tags=["Authentication"])
async def delete_user_me(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """
    Delete the current authenticated user's account.
    Requires: Bearer token in Authorization header
    Response: APIResponse with null data
    Raises: HTTP 401 if token is invalid or user not found
    """
    await delete_user(db, current_user)
    return {"status": "success", "data": None}

@router.put("/users/me/password", response_model=APIResponse[UserOut], tags=["Authentication"])
